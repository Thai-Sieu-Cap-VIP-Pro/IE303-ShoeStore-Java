...
import { fetchOrdersData, selectOrders } from "../../OrderManagement/OrderSlice";
...
const DashboardPage = () => {
 ...
 date6.setDate(date0.getDate() - 6);
  ...
  if(orders.length) {
    ...
    var d = new Date(item.orderDate.split("-")[0],Number(item.orderDate.split("-")[1]) -1,item.orderDate.split("-")[2])
    console.log(d)
    console.log(date)
